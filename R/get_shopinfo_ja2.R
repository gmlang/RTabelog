...
get_shopinfo_en = function(shopURL) {
  ...
  rating_dinner =  extract(".rdheader-rating__time-icon--dinner") %>%
    gsub(pattern="\n|夜の点数：", replace="") %>% as.numeric()
  rating_lunch = extract(".rdheader-rating__time-icon--lunch") %>%
  ...
  # take reservations
  reservation = ifelse(
    extract(".rstinfo-table__reserve-status") == "予約可",
    "Yes", "No")
  ...
  out = data.frame(shop_name, cuisine, rating_dinner, rating_lunch,
                   ...
                   stringsAsFactors = F)
  names(out) = c("店名", "ジャンル", "夜の点数",
                 "昼の点数", "口コミ", "夜の予算", "昼の予算",
                 "最寄り", "利用シーン",
                 "クレジットカード", "予約",
                 "個室", "喫煙・禁煙", "駐車場", "空間・設備",
                 "住所", "電話番号", "ホームページ", "食べログURL")
  out
}
